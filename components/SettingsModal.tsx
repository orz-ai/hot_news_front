...
export default function SettingsModal({ isOpen, onClose, settings, onSettingsChange }: SettingsModalProps) {
  ...
  const [activeTab, setActiveTab] = useState<'display' | 'platforms' | 'advanced'>('display');
  ...
  const handlePlatformToggle = (platformCode: PlatformType) => {
    const newFeaturedPlatforms = localSettings.featuredPlatforms.includes(platformCode)
      ? localSettings.featuredPlatforms.filter(p => p !== platformCode)
      : [...localSettings.featuredPlatforms, platformCode];
                  ...
                  <div className="space-y-6">
                        ...
                        {PLATFORMS.map((platform) => {
                          ...
                          return (
                            <button
                              ...
                              disabled={!isSelected && localSettings.featuredPlatforms.length >= 8}
                              className={`flex items-center gap-3 p-3 rounded-lg border transition-all ${
                                isSelected
                                  ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300'
                                  : 'border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600 text-gray-700 dark:text-gray-300'
                              } ${!isSelected && localSettings.featuredPlatforms.length >= 8 ? 'opacity-50 cursor-not-allowed' : ''}`}
                            >
                              <div
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        {localSettings.platformOrder.map((platformCode, index) => {
                          ...
                          return (
                            <div
                              key={platformCode}
                              draggable
                              onDragStart={() => handleDragStart(platformCode)}
                              onDragOver={handleDragOver}
                              onDrop={(e) => handleDrop(e, platformCode)}
